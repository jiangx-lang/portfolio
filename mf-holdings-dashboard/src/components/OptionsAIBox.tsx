...
export function OptionsAIBox({
  ...
  if (!contract) {
    return (
      <div className="rounded-xl border border-white/10 bg-navy-card p-4 text-sm text-white/60">
        点击 Options chain 里的 Call/Put Bid 选择一个合约，然后在这里生成合约级 AI 策略建议。
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-navy-card p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="text-sm font-semibold text-white/90">Options AI</h3>
          <p className="mt-1 text-xs text-white/50">{headline}</p>
        </div>
        <button
          type="button"
          onClick={analyze}
      ...
      {result && !loading && (
        <div className="mt-3 space-y-2 text-sm">
          <p className={cn("rounded border px-2 py-1 font-medium capitalize", SIGNAL_COLORS[result.signal])}>
            {result.signal.replace("_", " ")} · {result.confidence}% confidence
          </p>
          <p className="text-white/80">{result.thesis}</p>
          ...
          {result.optionsStrategy && (
            <div className="rounded border border-white/10 bg-white/5 p-2 text-xs">
              <p className="font-medium text-white/80">{result.optionsStrategy.recommended}</p>
              <p className="text-white/60">{result.optionsStrategy.rationale}</p>
